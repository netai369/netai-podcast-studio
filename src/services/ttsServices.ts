                    ...
                    if (jsonMatch) {
                        try {
                            ...
                            try {
                                ...
                                return {
                                    title: 'Podcast',
                                    description: 'A generated podcast'
                                };
                            }
                        }
                    }
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        try {
            if (conf.tts.provider === 'gemini') {
                ...
                const matchedSpeaker = (isValidNameLength && potentialSpeakerName) ? speakers.find(s => s.name === potentialSpeakerName) : undefined;
            ...
            if (audioB64) {
                ...
                } else if (conf.tts.provider === 'supertonic') {
        ...
        try {
            ...
            if (data.voices && Array.isArray(data.voices)) {
                return data.voices.map((voice: any) => ({
                    ...
                    name: voice.name,
                    gender: voice.gender || 'M',
                    label: `${voice.name}${voice.description ? ` - ${voice.description}` : ''}${voice.voice_type ? ` (${voice.voice_type})` : ''}`
                }));
            }

            return [];
        } catch (e) {
            console.error('DEBUG: Failed to fetch Supertonic voices:', e);
            if (e instanceof TypeError) {
                throw createCorsError(conf.tts.openAudioUrl);
            }
            throw e;
...
export const mixVoices = async (conf: BackendConfig, voiceA: string, voiceB: string, weight: number, name?: string): Promise<string> => {
    ...
    try {
        ...
        console.error('DEBUG: Failed to mix voices:', e);
        if (e instanceof TypeError) {
            throw createCorsError(conf.tts.openAudioUrl);
        }
        throw e;
